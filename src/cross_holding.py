...
def build_cross_holding_matrix(conn, report_period=None):
    ...
    # 从 DB 回读 turnover_proxy（如果已有），保持函数返回的 DataFrame 完整
    db_proxy = pd.read_sql_query("""
        SELECT institution_cik, turnover_proxy
        FROM cross_holding_matrix
        WHERE report_period = ?
    """, conn, params=(report_period,))
    if not db_proxy.empty:
        matrix = matrix.merge(db_proxy, on="institution_cik", how="left")
...
<style>
  body {{ font-family: -apple-system, "Segoe UI", "Helvetica Neue", sans-serif;
          max-width: 1200px; margin: 20px auto; padding: 0 20px; color: #1a1a1a; }}
  ...
  .disclaimer {{ background: #fff8e1; border-left: 4px solid #f59e0b;
                 padding: 12px; margin: 20px 0; font-size: 13px; color: #555; }}
  .chart {{ margin: 16px 0; }}
...
def _wrap_html(title, body):
    return HTML_HEADER.format(title=title, body=body)


def _fig_to_div(fig):
    """Plotly figure → embeddable HTML div (interactive)."""
    return fig.to_html(
        full_html=False,
...
def export_charts_html(conn, output_dir="output/preview"):
    """
    导出所有交叉持股图表为独立 HTML 文件 + 入口 index.html。

    文件清单：
      - index.html         入口页（链接到所有图表）
      - 01_top_holders.html    Top Holders 排名 + 热力图
      - 02_qoq_changes.html    QoQ 变动表
      - 03_top_buyers.html     Top Buyers
      - 04_top_sellers.html    Top Sellers
      - 05_init_liq.html       Initiations & Liquidations
      - 06_turnover.html       Turnover Proxy
      - 07_capital_flows.html  按风格/Turnover/Activism 归因图
    """
    ...
    matrix = build_cross_holding_matrix(conn)
    if matrix.empty:
        logger.warning("No cross-holding data. Nothing to export.")
        return
    ...
    latest_period = matrix["report_period"].iloc[0] if "report_period" in matrix.columns else "未知"
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
    meta = f'<div class="meta">报告期: <b>{latest_period}</b>  |  生成时间: {timestamp}</div>'

    # ── 1. Top Holders ──
    body = meta + '<h1>1. Top Holders 持仓排名</h1>'
    body += '<div class="disclaimer">⚠️ 风格标签为简化分类（非 IHS Markit 12 类专业分类）。机构覆盖 25 家种子中的 8 家。</div>'

    # 热力图
    heatmap_data = matrix[COMPETITOR_TICKERS].div(1000)
    fig = go.Figure(data=go.Heatmap(
        z=heatmap_data.values,
        x=COMPETITOR_TICKERS,
        y=matrix["institution_name"],
        colorscale="Blues",
        hovertemplate="机构: %{y}<br>竞品: %{x}<br>持仓: $%{z:,.1f}M<extra></extra>",
        colorbar=dict(title="$M"),
    ))
    fig.update_layout(
        height=max(300, 40 * len(matrix)),
        margin=dict(l=20, r=20, t=10, b=20),
        xaxis=dict(side="top", title=""),
        yaxis=dict(title="", tickfont=dict(size=11), autorange="reversed"),
    )
    body += '<div class="chart">' + _fig_to_div(fig) + '</div>'

    # 表格
    table_data = matrix[["institution_name", "style_label"] + COMPETITOR_TICKERS
                        + ["total_value_x1000", "peer_avg_x1000", "turnover_proxy"]].copy()
    body += '<table><thead><tr><th>机构</th><th>风格</th>'
    for tk in COMPETITOR_TICKERS:
        body += f'<th>{tk} ($M)</th>'
    body += '<th>总持仓 ($M)</th><th>Peer Avg ($M)</th><th>Turnover</th></tr></thead><tbody>'
    for _, r in table_data.iterrows():
        body += f'<tr><td>{r["institution_name"]}</td><td>{r["style_label"]}</td>'
        for tk in COMPETITOR_TICKERS:
            v = r[tk] / 1000 if pd.notna(r[tk]) and r[tk] > 0 else None
            body += f'<td>{f"${v:,.1f}" if v else "-"}</td>'
        total = r["total_value_x1000"] / 1000 if r["total_value_x1000"] > 0 else 0
        avg = r["peer_avg_x1000"] / 1000 if r["peer_avg_x1000"] > 0 else 0
        body += f'<td><b>${total:,.1f}</b></td><td>${avg:,.1f}</td><td>{r.get("turnover_proxy") or "-"}</td></tr>'
    body += '</tbody></table>'

    (output_dir / "01_top_holders.html").write_text(_wrap_html("Top Holders - 交叉持股分析", body))

    # ── 2. QoQ 变动 ──
    body = meta + '<h1>2. QoQ 持仓变动</h1>'
    if not qoq.empty:
        body += '<table><thead><tr><th>机构</th><th>竞品</th><th>当前 ($K)</th><th>上期 ($K)</th><th>Δ ($K)</th><th>Δ %</th></tr></thead><tbody>'
        for _, r in qoq.head(30).iterrows():
            d_pct = f'{r["delta_pct"]:+.1f}%' if pd.notna(r["delta_pct"]) else "-"
            body += f'<tr><td>{r["institution_name"]}</td><td>{r["ticker"]}</td>'
            body += f'<td>{r["current_value"]:,.0f}</td><td>{r["previous_value"]:,.0f}</td>'
            body += f'<td style="color:{"#22c55e" if r["delta_value"] > 0 else "#ef4444"}">{r["delta_value"]:+,.0f}</td>'
            body += f'<td>{d_pct}</td></tr>'
        body += '</tbody></table>'
    else:
        body += '<p>暂无 QoQ 变动数据（需要至少两期 13F）。</p>'
    (output_dir / "02_qoq_changes.html").write_text(_wrap_html("QoQ 变动 - 交叉持股分析", body))

    # ── 3. Top Buyers ──
    body = meta + '<h1>3. Top Peer Buyers（增持排名）</h1>'
    if not buyers.empty:
        body += '<table><thead><tr><th>#</th><th>机构</th><th>风格</th><th>总增持 ($M)</th>'
        for tk in COMPETITOR_TICKERS:
            body += f'<th>{tk} ($M)</th>'
        body += '</tr></thead><tbody>'
        for _, r in buyers.iterrows():
            body += f'<tr><td>{int(r["rank"])}</td><td>{r["institution_name"]}</td><td>{r["style_label"]}</td>'
            body += f'<td><b>+${r["total_change"]/1000:,.1f}</b></td>'
            for tk in COMPETITOR_TICKERS:
                v = r.get(f"{tk}_change", 0) / 1000
                body += f'<td>{"+%.1f" % v if v > 0 else "-"}</td>'
            body += '</tr>'
        body += '</tbody></table>'
    else:
        body += '<p>暂无数据。</p>'
    (output_dir / "03_top_buyers.html").write_text(_wrap_html("Top Buyers - 交叉持股分析", body))

    # ── 4. Top Sellers ──
    body = meta + '<h1>4. Top Peer Sellers（减持排名）</h1>'
    if not sellers.empty:
        body += '<table><thead><tr><th>#</th><th>机构</th><th>风格</th><th>总减持 ($M)</th>'
        for tk in COMPETITOR_TICKERS:
            body += f'<th>{tk} ($M)</th>'
        body += '</tr></thead><tbody>'
        for _, r in sellers.iterrows():
            ...
            body += f'<td><b>${r["total_change"]/1000:,.1f}</b></td>'
            for tk in COMPETITOR_TICKERS:
                v = abs(r.get(f"{tk}_change", 0)) / 1000
                body += f'<td>{v:.1f}</td>' if v > 0 else '<td>-</td>'
            body += '</tr>'
        body += '</tbody></table>'
    else:
        body += '<p>暂无数据。</p>'
    (output_dir / "04_top_sellers.html").write_text(_wrap_html("Top Sellers - 交叉持股分析", body))

    # ── 5. Initiations & Liquidations ──
    body = meta + '<h1>5. Initiations & Liquidations</h1>'

    body += '<h2>5.1 新建仓 (Initiations)</h2>'
    if not init_df.empty:
        body += '<table><thead><tr><th>#</th><th>机构</th><th>竞品</th><th>市值 ($M)</th><th>风格</th></tr></thead><tbody>'
        for i, (_, r) in enumerate(init_df.head(15).iterrows(), 1):
            v = r["current_value"] / 1000
            body += f'<tr><td>{i}</td><td>{r["institution_name"]}</td><td>{r["ticker"]}</td>'
            body += f'<td>${v:,.1f}</td><td>{r["style_label"]}</td></tr>'
        body += '</tbody></table>'
    else:
        body += '<p>暂无新建仓。</p>'

    body += '<h2>5.2 清仓 (Liquidations)</h2>'
    if not liq_df.empty:
        body += '<table><thead><tr><th>#</th><th>机构</th><th>竞品</th><th>上期市值 ($M)</th><th>风格</th></tr></thead><tbody>'
        for i, (_, r) in enumerate(liq_df.head(15).iterrows(), 1):
            v = r["previous_value"] / 1000
            body += f'<tr><td>{i}</td><td>{r["institution_name"]}</td><td>{r["ticker"]}</td>'
            body += f'<td>${v:,.1f}</td><td>{r["style_label"]}</td></tr>'
        body += '</tbody></table>'
    else:
    ...
    (output_dir / "05_init_liq.html").write_text(_wrap_html("Init/Liq - 交叉持股分析", body))

    # ── 6. Turnover Proxy ──
    body = meta + '<h1>6. 持仓变动率估算 (Turnover Proxy)</h1>'
    body += '<div class="disclaimer">⚠️ 基于 QoQ 13F 快照的 churn proxy，非精确 portfolio turnover。3 档分类（Low/Medium/High）。</div>'

    if not turnover.empty:
        body += '<table><thead><tr><th>机构</th><th>Churn Proxy</th><th>分档</th><th>风格</th></tr></thead><tbody>'
        for _, r in turnover.iterrows():
            body += f'<tr><td>{r["institution_name"]}</td><td>{r["churn_proxy"]:.1f}%</td>'
            body += f'<td>{r["churn_label"]}</td><td>{r["style_label"]}</td></tr>'
        body += '</tbody></table>'

    (output_dir / "06_turnover.html").write_text(_wrap_html("Turnover - 交叉持股分析", body))

    # ── 7. 资本流向归因图 ──
    body = meta + '<h1>7. 资本流向归因（按分类）</h1>'
    body += '<div class="disclaimer">⚠️ 基于简化分类（非 IHS Markit 12 类专业分类）。数据源：QoQ 13F 持仓变动。</div>'

    def _bar_html(df, group_col, title):
        if df is None or df.empty:
            return f'<p>{title}：暂无数据</p>'
        ...
        fig.update_layout(
            ...
            xaxis=dict(title="QoQ 资金流向 ($M)"),
            yaxis=dict(title=""),
            showlegend=False,
        )
        return '<div class="chart">' + _fig_to_div(fig) + '</div>'

    body += _bar_html(flows.get("by_style"), "style_label", "按投资风格 (Style)")
    body += _bar_html(flows.get("by_turnover"), "turnover_label", "按 Turnover")
    body += _bar_html(flows.get("by_activism"), "activism", "按 Activism")

    (output_dir / "07_capital_flows.html").write_text(_wrap_html("资本流向归因 - 交叉持股分析", body))
    ...
    <div class="disclaimer">
      ⚠️ 免责声明：风格标签为简化分类（非 IHS Markit 12 类专业分类）。
      机构覆盖 25 家种子机构中持有竞品的 8 家。13F 数据滞后约 45 天。
    </div>
    <h2>📊 图表导航</h2>
    <ul style="line-height: 2;">
      <li><a href="01_top_holders.html">1. Top Holders 持仓排名（含热力图）</a></li>
      <li><a href="02_qoq_changes.html">2. QoQ 持仓变动表</a></li>
      <li><a href="03_top_buyers.html">3. Top Peer Buyers（增持排名）</a></li>
      <li><a href="04_top_sellers.html">4. Top Peer Sellers（减持排名）</a></li>
      <li><a href="05_init_liq.html">5. Initiations &amp; Liquidations（新建仓 / 清仓）</a></li>
      <li><a href="06_turnover.html">6. 持仓变动率估算 (Turnover Proxy)</a></li>
      <li><a href="07_capital_flows.html">7. 资本流向归因（按 Style / Turnover / Activism）</a></li>
    </ul>
    <h2>📁 配套交付物</h2>
    <ul style="line-height: 2;">
      <li><a href="../cross_holding_report.md">📄 Markdown 报告</a></li>
      <li><a href="../../prd/gap-analysis-ihs-markit.md">📋 差距分析文档（IHS Markit）</a></li>
    </ul>
    """
    (output_dir / "index.html").write_text(_wrap_html("竞品交叉持股分析预览", index_body))

    # ── 列出输出文件 ──
    files = sorted(output_dir.glob("*.html"))
    logger.info("Exported %d HTML files to %s:", len(files), output_dir)
    for f in files:
